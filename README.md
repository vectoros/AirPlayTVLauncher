...
静态照片等待期间不重绘壁纸或采样玻璃，过渡、推拉与程序化背景最高 25fps；离开首页暂停动画。低配设备仍可能受到多卡片模糊、主线程应用扫描和 4K 绘制影响。关闭液态玻璃会恢复旧毛玻璃，**不等于关闭模糊**。本次性能优化尚未真机复测。

逐版本限制、性能排查和采样命令见 [兼容性与性能](docs/COMPATIBILITY_AND_PERFORMANCE.md)。
